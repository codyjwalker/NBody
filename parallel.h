 ...
 * Description:   Header file for the parallel implementation of the n-body
...
extern void calculate_forces(int worker);
extern void move_bodies(int worker);
extern int collisions_detected(void);
extern void resolve_collisions(void);
extern void export_positions(void);
extern void fork_recursion(int num);
extern void barrier(int worker);
...
int num_workers;          // Number of worker processes to use in simulation.
int num_phases;           // Number of phases needed for the barrier.
...
sem_t *sem_table[MAX_PHASES][MAX_WORKERS];  // Semaphore table.
