 ...
 * Description:     A combination of all previous C implementations of the NBody
 *	 	            gravitational simulation.  Combines the features of the 2D
 *	 	            sequential version, the 2D parallel version, and both 3D
 *	 	            versions in one file, so that rather than the user building
 *	 	            and running the appropriate C program, they simply select
 *	 	            which options they'd like in the GUI window and the program
 *	 	            will run the simulaion in either of the four methods.
...
// TODO: CHANGE UP HEADER FILE!!!!!
#include "3dparallel.h"
...
void calculate_forces(int worker)
{
    ...
    for (i = 0; i < num_bodies - 1; i++)
    {
        for (j = i + 1; j < num_bodies; j++)
        {
            ...
            // TODO: WHAT IS THIS??? IMPLEMENTED ABOVE????
...
void resolve_collisions(void)
{
    ...
    while (collisions.size > 0)
    {
        ...
        // TODO: GET RID OF pow() AND SPEED THIS SHIT UP FOOL!
        // Precompute similar "chunks" used in all eqs to reduce computations.
